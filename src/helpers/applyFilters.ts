...
export const applyFiltersBikes = (bikes: Bike[], filters: BikeFilters): Bike[] => {
  let filtered = [...bikes];

  if (filters.brand && filters.brand.length > 0) {
    filtered = filtered.filter(
      (b) => filters.brand!.includes(b.brand?.name || '') || filters.brand!.includes(b.brand?.id || ''),
    );
  }

  if (filters.riding_type && filters.riding_type.length > 0) {
    filtered = filtered.filter((b) => filters.riding_type!.includes(b.riding_type || ''));
  }

  if (filters.frame_material && filters.frame_material.length > 0) {
    filtered = filtered.filter(
      (b) => filters.frame_material!.includes(b.frame_material || ''),
    );
  }
...
export const applyBrandFilters = (brands: Brand[], filters: BrandFilters): Brand[] => {
  return brands.filter((brand) => {
    if (filters.name && !brand.name.toLowerCase().includes(filters.name.toLowerCase())) {
      return false;
    }
    if (filters.country && brand.country?.name.toLowerCase() !== filters.country.toLowerCase()) {
      return false;
    }
